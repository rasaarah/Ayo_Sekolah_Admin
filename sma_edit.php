...
?>
                ...
                if(isset($_POST['submit'])){
                  ...
                  if(!$error){
                    if(!empty($id_kecamatan)  && !empty($nama) && !empty($alamat)  && !empty($lat) && !empty($lng)){
                         $queryupd = mysql_query("UPDATE sma SET id_kecamatan='".$id_kecamatan."', nama='".$nama."'
                          , alamat='".$alamat."',lat='".$lat."',lng='".$lng."',c1='".$c1."',c2='".$c2."',c3='".$c3."' WHERE id_sma='".$id."'");
                   
                      if($queryupd){
                      echo"<script>alert('Daftar sekolah terupdate.'); location='sma.php'</script>";
                      }else{
                 ...
                 if(isset($_GET['id_sma'])){
                  $sql_id = mysql_query("SELECT * FROM sma WHERE id_sma = '".$_GET['id_sma']."'");
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Edit SMA</h1>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                <!-- /.row -->
